...
def get_full_grad_list(net, train_set, optimizer, batch_size=1000):
...
def save_omega(omega):
    os.makedirs('./omega-sgd', exist_ok=True)                                                                                                                                                                
    numpy_gradients = [g.cpu().numpy() for g in saved_gradients]                                                                                                 
    flat_gradients = [grad.flatten() for grad in numpy_gradients]                                                                                                                  
    concatenated_gradients = np.concatenate(flat_gradients, axis=0)                                                                                                                     
    np.save(f'./omega-sgd/omega.npy', concatenated_gradients)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--optimizer', default="sgd", type=str, help="sgd, shb, nshb")
    parser.add_argument('--method', default="sampling", type=str, help="normal, critical, sampling")
