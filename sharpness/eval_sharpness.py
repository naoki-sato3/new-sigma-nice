...
def get_args():
    ...
    parser.add_argument('--model', default='resnet18_plain', type=str)
    parser.add_argument('--model_path', default='./sgd-200/sgd4096-200.pth', type=str, help='model path')
    ...
    parser.add_argument('--rho', default=0.0005, type=float, help='L2 radius for sharpness')
    ...
    parser.add_argument('--batch', type=int, help='batch size used in training')
...
def convert_state_dict(state_dict):
    ...
    for k, v in state_dict.items():
        ...
        # residual_functionをconvに置換
        new_key = new_key.replace('residual_function', 'conv')
        
        # conv{n}_xをlayer{n}に置換
        new_key = new_key.replace('conv2_x', 'layer1')
        new_key = new_key.replace('conv3_x', 'layer2')
        new_key = new_key.replace('conv4_x', 'layer3')
        new_key = new_key.replace('conv5_x', 'layer4')
...
wandb_exp_name = f"sgd,{args.batch}"
...
#model_dict = models.get_model(args.model, n_cls, False, data.shapes_dict[args.dataset], args.model_width, normalize_logits=args.normalize_logits).to(device).eval()
model_dict = torch.load('{}'.format(args.model_path))
#print(model_test.keys())
#model_test = torch.load(args.model_path)
#model_dict.load_state_dict(model_test)
# キーを変換
model_dict = convert_state_dict(model_dict)

# モデルの状態をロード
#model_dict.load_state_dict(converted_state_dict)
